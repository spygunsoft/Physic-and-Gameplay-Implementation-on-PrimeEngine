...
#include "PrimeEngine/Physic/PhysicsManager.h"
...
int ClientGame::runGameFrame()
{
	...
	{
		Handle hprePhysicsEvent("EVENT", sizeof(Event_PRE_PHYSIC));
		Event_PRE_PHYSIC *prePhysicsEvent = new(hprePhysicsEvent) Event_PRE_PHYSIC;

		Events::EventQueueManager::Instance()->add(hprePhysicsEvent, Events::QT_GENERAL);
	}

	{
		Handle hupdatePhysicsEvent("EVENT", sizeof(Event_UPDATE_PHYSIC));
		Event_UPDATE_PHYSIC *updatePhysicsEvent = new(hupdatePhysicsEvent) Event_UPDATE_PHYSIC;

		updatePhysicsEvent->m_frameTime = m_frameTime;

		Events::EventQueueManager::Instance()->add(hupdatePhysicsEvent, Events::QT_GENERAL);
	}
    ...
    while (!gqh.getObject<Events::EventQueue>()->empty())
    {
        ...
        else if (Event_CALCULATE_TRANSFORMATIONS::GetClassId() == pGeneralEvt->getClassId())
        {
            ...
            // Push DRAW event into message queue because camera has updated transformations
            {
				...
				drawEvt->m_frontPlaneNormal = pcam->frontPlaneNormal;
				drawEvt->m_frontPlaneOffset = pcam->frontPlaneOffset;

				drawEvt->m_leftPlaneNormal = pcam->leftPlaneNormal;
				drawEvt->m_leftPlaneOffset = pcam->leftPlaneOffset;

				drawEvt->m_bottomPlaneNormal = pcam->bottomPlaneNormal;
				drawEvt->m_bottomPlaneOffset = pcam->bottomPlaneOffset;

				drawEvt->m_rightPlaneNormal = pcam->rightPlaneNormal;
				drawEvt->m_rightPlaneOffset = pcam->rightPlaneOffset;

				drawEvt->m_topPlaneNormal = pcam->topPlaneNormal;
				drawEvt->m_topPlaneOffset = pcam->topPlaneOffset;

				drawEvt->m_backPlaneNormal = pcam->backPlaneNormal;
				drawEvt->m_backPlaneOffset = pcam->backPlaneOffset;
		...
		else if (Event_PRE_PHYSIC::GetClassId() == pGeneralEvt->getClassId())
		{
			m_pContext->getPhysicsManager()->handleEvent(pGeneralEvt);
		}

		else if (Event_UPDATE_PHYSIC::GetClassId() == pGeneralEvt->getClassId()) 
		{
			m_pContext->getPhysicsManager()->handleEvent(pGeneralEvt);
		}
			...
			|| Event_GATHER_DRAWCALLS_Z_ONLY::GetClassId() == pGeneralEvt->getClassId())
        {
			...
			if (!zOnly)
			{
				...
				//FPS
				{
					...
					sprintf(PEString::s_buf, "%.2f anugroho FALL 2016 FPS", fps);
					DebugRenderer::Instance()->createTextMesh(
						PEString::s_buf, true, false, false, false, 0, 
						Vector3(.7f, .05f, 0), 1.0f, m_pContext->m_gameThreadThreadOwnershipMask);
				}
