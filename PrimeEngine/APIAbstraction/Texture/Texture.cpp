...
#	if APIABSTRACTION_X360
	...
	do {
		...
		hr = D3DXCreateTextureFromFileA(pD3D9Renderer->m_pD3D9Device, PEString::s_buf, &m_pTexture);
	} while (PEASSERT(SUCCEEDED(hr), "Could not create a texture from path: %s\nFile might be missing. Check and retry.", PEString::s_buf));
	
	if (FAILED(hr))
	{
		// texture was not loaded
		createTextureNoFamily("MISSING_ASSET.dds", "Default"); // load in a debug texture
	}
...
#	if APIABSTRACTION_X360
	...
	// retrieve device from IRenderer
	HRESULT hr = D3DXCreateCubeTextureFromFileA(pD3D9Renderer->m_pD3D9Device, PEString::s_buf, &m_pCubeTexture);
	assert(SUCCEEDED(hr));
