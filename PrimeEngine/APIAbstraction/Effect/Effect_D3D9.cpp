...
void Effect::loadTechniqueSync_D3D9()
{
	...
	LPD3DXBUFFER pVertexShaderBuffer = NULL;
	LPD3DXBUFFER pPixelShaderBuffer = NULL;
#else
...
#ifdef _XBOX
	D3DXMACRO Shader_Macros[4] = { {"HLSL_SEPARATE_LOAD", "1"} , {"APIABSTRACTION_D3D9", "1"}, {"APIABSTRACTION_X360", "1"} , {0, 0}};
#else
	...
	{
#if APIABSTRACTION_X360
		loadShader_D3D9_X360(m_vsFilename, m_vsName, Shader_Macros,
	...
	if (StringOps::length(m_psFilename))
	{
		...
		*/
		{
#if APIABSTRACTION_X360
			loadShader_D3D9_X360(m_psFilename, m_psName, Shader_Macros,
	...
	if (pVertexShaderBuffer)
	{
		...
		{
#ifdef _XBOX
			PEINFO("PYENGINE ERROR: Failed To Create Vertex Shader (it did compile though) \
							   \nError hr = E_OUTOFMEMORY");
#else
		...
		{
#ifdef _XBOX
			PEINFO("PE: ERROR: Failed To Create Vertex Shader (it did compile though) \
							   \nError hr = D3DERR_OUTOFVIDEOMEMORY");
#else
			MessageBoxA(0, "Failed To Create Vertex Shader (it did compile though) \
...
#if APIABSTRACTION_X360
void Effect::loadShader_D3D9_X360(const char *filename, const char *name,
						D3DXMACRO *pMacros,
						const char *version,
						LPD3DXBUFFER &pShaderBuffer)
{
	...
	LPD3DXBUFFER pErrorBlob;
	LPD3DXCONSTANTTABLE pCTable;
