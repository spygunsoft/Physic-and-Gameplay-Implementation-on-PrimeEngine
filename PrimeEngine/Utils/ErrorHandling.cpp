...
{ 
	...
	if (!(expr))
	{
		...
		assert(false);
#ifdef _XBOX
		//OutputDebugStringA("PyEngine Error");
		//OutputDebugStringA("\n");
		return false; // cant do anything since no popup window. will not retry
#elif !APIABSTRACTION_IOS && !APIABSTRACTION_PS3 && !PE_PLAT_IS_PSVITA
...
PEERROR(	const char *format, ...)
{
	...
	//OutputDebugStringA("\n");
#elif !APIABSTRACTION_PS3 && !APIABSTRACTION_IOS && !PE_PLAT_IS_PSVITA
	switch (MessageBoxA(0, buf, "PyEngine Error", MB_ABORTRETRYIGNORE))
	{
	case IDABORT:
		exit(-1);
