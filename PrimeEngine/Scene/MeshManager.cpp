...
PE::Handle MeshManager::getAsset(const char *asset, const char *package, int &threadOwnershipMask)
{
	...
	else if (StringOps::endswith(asset, "mesha"))
	{
		...
		PositionBufferCPU *pBC = pMesh->m_hPositionBufferCPU.getObject<PositionBufferCPU>();
		pMesh->bBmin.m_x = pBC->m_values[0];
		pMesh->bBmin.m_y = pBC->m_values[1];
		pMesh->bBmin.m_z = pBC->m_values[2];

		pMesh->bBmax.m_x = pBC->m_values[0];
		pMesh->bBmax.m_y = pBC->m_values[1];
		pMesh->bBmax.m_z = pBC->m_values[2];
		
		for (int i = 3; i < pBC->m_values.m_size; i += 3)
		{
			pMesh->bBmin.m_x = pMesh->bBmin.m_x < pBC->m_values[i]? pMesh->bBmin.m_x : pBC->m_values[i];
			pMesh->bBmin.m_y = pMesh->bBmin.m_y < pBC->m_values[i + 1] ? pMesh->bBmin.m_y : pBC->m_values[i + 1];
			pMesh->bBmin.m_z = pMesh->bBmin.m_z < pBC->m_values[i + 2] ? pMesh->bBmin.m_z : pBC->m_values[i + 2];

			pMesh->bBmax.m_x = pMesh->bBmax.m_x > pBC->m_values[i] ? pMesh->bBmax.m_x : pBC->m_values[i];
			pMesh->bBmax.m_y = pMesh->bBmax.m_y > pBC->m_values[i + 1] ? pMesh->bBmax.m_y : pBC->m_values[i + 1];
			pMesh->bBmax.m_z = pMesh->bBmax.m_z > pBC->m_values[i + 2] ? pMesh->bBmax.m_z : pBC->m_values[i + 2];
		}
