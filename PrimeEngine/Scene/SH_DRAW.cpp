...
void SingleHandler_DRAW::do_GATHER_DRAWCALLS(Events::Event *pEvt)
{
    ...
    if (pMeshCaller->m_performBoundingVolumeCulling)
    {
        ...
        for (int iInst = 0; iInst < pMeshCaller->m_instances.m_size; ++iInst)
        {
			...
			if (pDrawEvent == NULL) continue;


			bool m_checkfbl = pDrawEvent->m_frontPlaneNormal.dotProduct(pInst->m_fbl) - pDrawEvent->m_frontPlaneOffset > 0
				&&
				pDrawEvent->m_leftPlaneNormal.dotProduct(pInst->m_fbl) - pDrawEvent->m_leftPlaneOffset > 0
				&&
				pDrawEvent->m_bottomPlaneNormal.dotProduct(pInst->m_fbl) - pDrawEvent->m_bottomPlaneOffset > 0
				&&
				pDrawEvent->m_rightPlaneNormal.dotProduct(pInst->m_fbl) - pDrawEvent->m_rightPlaneOffset > 0
				&&
				pDrawEvent->m_topPlaneNormal.dotProduct(pInst->m_fbl) - pDrawEvent->m_topPlaneOffset > 0
				&&
				pDrawEvent->m_backPlaneNormal.dotProduct(pInst->m_fbl) - pDrawEvent->m_backPlaneOffset > 0;
			
			bool m_checkbtr = pDrawEvent->m_frontPlaneNormal.dotProduct(pInst->m_btr) - pDrawEvent->m_frontPlaneOffset > 0
				&&
				pDrawEvent->m_leftPlaneNormal.dotProduct(pInst->m_btr) - pDrawEvent->m_leftPlaneOffset > 0
				&&
				pDrawEvent->m_bottomPlaneNormal.dotProduct(pInst->m_btr) - pDrawEvent->m_bottomPlaneOffset > 0
				&&
				pDrawEvent->m_rightPlaneNormal.dotProduct(pInst->m_btr) - pDrawEvent->m_rightPlaneOffset > 0
				&&
				pDrawEvent->m_topPlaneNormal.dotProduct(pInst->m_btr) - pDrawEvent->m_topPlaneOffset > 0
				&&
				pDrawEvent->m_backPlaneNormal.dotProduct(pInst->m_btr) - pDrawEvent->m_backPlaneOffset > 0;

			bool m_checkfbr = pDrawEvent->m_frontPlaneNormal.dotProduct(pInst->m_fbr) - pDrawEvent->m_frontPlaneOffset > 0
				&&
				pDrawEvent->m_leftPlaneNormal.dotProduct(pInst->m_fbr) - pDrawEvent->m_leftPlaneOffset > 0
				&&
				pDrawEvent->m_bottomPlaneNormal.dotProduct(pInst->m_fbr) - pDrawEvent->m_bottomPlaneOffset > 0
				&&
				pDrawEvent->m_rightPlaneNormal.dotProduct(pInst->m_fbr) - pDrawEvent->m_rightPlaneOffset > 0
				&&
				pDrawEvent->m_topPlaneNormal.dotProduct(pInst->m_fbr) - pDrawEvent->m_topPlaneOffset > 0
				&&
				pDrawEvent->m_backPlaneNormal.dotProduct(pInst->m_fbr) - pDrawEvent->m_backPlaneOffset > 0;

			bool m_checkftl = pDrawEvent->m_frontPlaneNormal.dotProduct(pInst->m_ftl) - pDrawEvent->m_frontPlaneOffset > 0
				&&
				pDrawEvent->m_leftPlaneNormal.dotProduct(pInst->m_ftl) - pDrawEvent->m_leftPlaneOffset > 0
				&&
				pDrawEvent->m_bottomPlaneNormal.dotProduct(pInst->m_ftl) - pDrawEvent->m_bottomPlaneOffset > 0
				&&
				pDrawEvent->m_rightPlaneNormal.dotProduct(pInst->m_ftl) - pDrawEvent->m_rightPlaneOffset > 0
				&&
				pDrawEvent->m_topPlaneNormal.dotProduct(pInst->m_ftl) - pDrawEvent->m_topPlaneOffset > 0
				&&
				pDrawEvent->m_backPlaneNormal.dotProduct(pInst->m_ftl) - pDrawEvent->m_backPlaneOffset > 0;

			bool m_checkftr = pDrawEvent->m_frontPlaneNormal.dotProduct(pInst->m_ftr) - pDrawEvent->m_frontPlaneOffset > 0
				&&
				pDrawEvent->m_leftPlaneNormal.dotProduct(pInst->m_ftr) - pDrawEvent->m_leftPlaneOffset > 0
				&&
				pDrawEvent->m_bottomPlaneNormal.dotProduct(pInst->m_ftr) - pDrawEvent->m_bottomPlaneOffset > 0
				&&
				pDrawEvent->m_rightPlaneNormal.dotProduct(pInst->m_ftr) - pDrawEvent->m_rightPlaneOffset > 0
				&&
				pDrawEvent->m_topPlaneNormal.dotProduct(pInst->m_ftr) - pDrawEvent->m_topPlaneOffset > 0
				&&
				pDrawEvent->m_backPlaneNormal.dotProduct(pInst->m_ftr) - pDrawEvent->m_backPlaneOffset > 0;

			bool m_checkbbl = pDrawEvent->m_frontPlaneNormal.dotProduct(pInst->m_bbl) - pDrawEvent->m_frontPlaneOffset > 0
				&&
				pDrawEvent->m_leftPlaneNormal.dotProduct(pInst->m_bbl) - pDrawEvent->m_leftPlaneOffset > 0
				&&
				pDrawEvent->m_bottomPlaneNormal.dotProduct(pInst->m_bbl) - pDrawEvent->m_bottomPlaneOffset > 0
				&&
				pDrawEvent->m_rightPlaneNormal.dotProduct(pInst->m_bbl) - pDrawEvent->m_rightPlaneOffset > 0
				&&
				pDrawEvent->m_topPlaneNormal.dotProduct(pInst->m_bbl) - pDrawEvent->m_topPlaneOffset > 0
				&&
				pDrawEvent->m_backPlaneNormal.dotProduct(pInst->m_bbl) - pDrawEvent->m_backPlaneOffset > 0;

			bool m_checkbbr = pDrawEvent->m_frontPlaneNormal.dotProduct(pInst->m_bbr) - pDrawEvent->m_frontPlaneOffset > 0
				&&
				pDrawEvent->m_leftPlaneNormal.dotProduct(pInst->m_bbr) - pDrawEvent->m_leftPlaneOffset > 0
				&&
				pDrawEvent->m_bottomPlaneNormal.dotProduct(pInst->m_bbr) - pDrawEvent->m_bottomPlaneOffset > 0
				&&
				pDrawEvent->m_rightPlaneNormal.dotProduct(pInst->m_bbr) - pDrawEvent->m_rightPlaneOffset > 0
				&&
				pDrawEvent->m_topPlaneNormal.dotProduct(pInst->m_bbr) - pDrawEvent->m_topPlaneOffset > 0
				&&
				pDrawEvent->m_backPlaneNormal.dotProduct(pInst->m_bbr) - pDrawEvent->m_backPlaneOffset > 0;

			bool m_checkbtl = pDrawEvent->m_frontPlaneNormal.dotProduct(pInst->m_btl) - pDrawEvent->m_frontPlaneOffset > 0
				&&
				pDrawEvent->m_leftPlaneNormal.dotProduct(pInst->m_btl) - pDrawEvent->m_leftPlaneOffset > 0
				&&
				pDrawEvent->m_bottomPlaneNormal.dotProduct(pInst->m_btl) - pDrawEvent->m_bottomPlaneOffset > 0
				&&
				pDrawEvent->m_rightPlaneNormal.dotProduct(pInst->m_btl) - pDrawEvent->m_rightPlaneOffset > 0
				&&
				pDrawEvent->m_topPlaneNormal.dotProduct(pInst->m_btl) - pDrawEvent->m_topPlaneOffset > 0
				&&
				pDrawEvent->m_backPlaneNormal.dotProduct(pInst->m_btl) - pDrawEvent->m_backPlaneOffset > 0;

            if (m_checkbbl || m_checkbbr || m_checkbtl || m_checkbtr || m_checkfbl || m_checkfbr || m_checkftl || m_checkftr)
            {
