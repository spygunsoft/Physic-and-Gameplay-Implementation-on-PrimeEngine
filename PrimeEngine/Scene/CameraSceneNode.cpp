...
void CameraSceneNode::do_CALCULATE_TRANSFORMATIONS(Events::Event *pEvt)
{
	...
	Vector3 right = Vector3(mref_worldTransform.m[0][0], mref_worldTransform.m[1][0], mref_worldTransform.m[2][0]);
	...
	Vector3 right2 = Vector3(m_worldTransform2.m[0][0], m_worldTransform2.m[1][0], m_worldTransform2.m[2][0]);
	...
	Vector3 nearCenter = pos + n * m_near;
	Vector3 farCenter = pos + n * m_far;
	PrimitiveTypes::Float32 nearHeight = 2 * tan((verticalFov) / 2)*m_near;
	PrimitiveTypes::Float32 farHeight = 2 * tan((verticalFov) / 2)*m_far;
	PrimitiveTypes::Float32 nearWidth = nearHeight * aspect;
	PrimitiveTypes::Float32 farWidth = farHeight  * aspect;

	Vector3 ftl = farCenter + up*(farHeight*0.5) - right*(farWidth*0.5);
	Vector3 ftr = farCenter + up*(farHeight*0.5) + right*(farWidth*0.5);
	Vector3 fbl = farCenter - up*(farHeight*0.5) - right*(farWidth*0.5);
	Vector3 fbr = farCenter - up*(farHeight*0.5) + right*(farWidth*0.5);
	
	Vector3 ntl = nearCenter + up*(nearHeight*0.5) - right*(nearWidth*0.5);
	Vector3 ntr = nearCenter + up*(nearHeight*0.5) + right*(nearWidth*0.5);
	Vector3 nbl = nearCenter - up*(nearHeight*0.5) - right*(nearWidth*0.5);
	Vector3 nbr = nearCenter - up*(nearHeight*0.5) + right*(nearWidth*0.5);
	 
	frontPlaneNormal = ((nbl - ntl).crossProduct((ntr - ntl)));
	frontPlaneNormal.normalize();
	frontPlaneOffset = frontPlaneNormal.dotProduct(ntl);

	backPlaneNormal = ((ftr - ftl).crossProduct((fbl - ftl)));
	backPlaneNormal.normalize();
	backPlaneOffset = backPlaneNormal.dotProduct(ftl);
	
	leftPlaneNormal = ((ntl - nbl).crossProduct((fbl - nbl)));
	leftPlaneNormal.normalize();
	leftPlaneOffset = leftPlaneNormal.dotProduct(fbl);

	bottomPlaneNormal = ((nbl - nbr).crossProduct((fbr - nbr)));
	bottomPlaneNormal.normalize();
	bottomPlaneOffset = bottomPlaneNormal.dotProduct(fbr);

	rightPlaneNormal = ((nbr - ntr).crossProduct((ftr - ntr)));
	rightPlaneNormal.normalize();
	rightPlaneOffset = rightPlaneNormal.dotProduct(ftr);

	topPlaneNormal = ((ntr - ntl).crossProduct((ftl - ntl)));
	topPlaneNormal.normalize();
	topPlaneOffset = topPlaneNormal.dotProduct(ftl);
