...
public:
  ...
  NodeType getElementType(const char *str)
  {
  	NodeType ret = NT_NONE;
		...
		if ( _stricmp(str,"SceneGraph") == 0 )  ret = NT_SCENE_GRAPH;
		else if ( _stricmp(str,"Mesh") == 0 )  ret = NT_MESH;
		else if ( _stricmp(str,"Animation") == 0 )  ret = NT_ANIMATION;
		else if ( _stricmp(str,"Skeleton") == 0 )  ret = NT_SKELETON;
		else if ( _stricmp(str,"Bone") == 0 )  ret = NT_BONE;
		else if ( _stricmp(str,"MeshSection") == 0 )  ret = NT_MESH_SECTION;
		else if ( _stricmp(str,"VertexBuffer") == 0 )  ret = NT_VERTEX_BUFFER;
		else if ( _stricmp(str,"TetraMesh") == 0 )  ret = NT_NODE_TETRA_MESH;
		else if ( _stricmp(str,"IndexBuffer") == 0 )  ret = NT_INDEX_BUFFER;
		else if ( _stricmp(str,"NodeTriangle") == 0 )  ret = NT_NODE_TRIANGLE;
		else if ( _stricmp(str,"NodeInstance") == 0 )  ret = NT_NODE_INSTANCE;
		else if ( _stricmp(str,"AnimTrack") == 0 )  ret = NT_ANIM_TRACK;
	...
	AttributeType getAttributeType(const char *str)
	{
		...
		if ( _stricmp(str,"name") == 0 ) ret = AT_NAME;
		else if ( _stricmp(str,"count") == 0 ) ret = AT_COUNT;
		else if ( _stricmp(str,"parent") == 0 ) ret = AT_PARENT;
		else if ( _stricmp(str,"material") == 0 ) ret = AT_MATERIAL;
		else if ( _stricmp(str,"ctype") == 0 ) ret = AT_CTYPE;
		else if ( _stricmp(str,"semantic") == 0 ) ret = AT_SEMANTIC;
		else if ( _stricmp(str,"position") == 0 ) ret = AT_POSITION;
		else if ( _stricmp(str,"orientation") == 0 ) ret = AT_ORIENTATION;
		else if ( _stricmp(str,"duration") == 0 ) ret = AT_DURATION;
		else if ( _stricmp(str,"dtime") == 0 ) ret = AT_DTIME;
		else if ( _stricmp(str,"trackcount") == 0 ) ret = AT_TRACK_COUNT;
		else if ( _stricmp(str,"framecount") == 0 ) ret = AT_FRAME_COUNT;
			...
			case TiXmlNode::TEXT:
				...
				switch ( mType )
				{
					...
					case NT_NODE_TRIANGLE:
						if ( mCtype && mSemantic )
						{
							if ( _stricmp(mSemantic,"position normal texcoord texcoord blendweights blendindices") == 0 )
							{
								NxuDeformVertex vtx[3];
								Asc2Bin(svalue, 3, mCtype, vtx );
								mCallback->NodeTriangle( &vtx[0], &vtx[1], &vtx[2] );
							}
							else if ( _stricmp(mSemantic,"position normal texcoord texcoord") == 0 )
							{
								NxuVertex vtx[3];
								Asc2Bin(svalue, 3, mCtype, vtx );
								mCallback->NodeTriangle( &vtx[0], &vtx[1], &vtx[2] );
            ...
            break;
					...
					case NT_INDEX_BUFFER:
						...
						if ( mIndexBuffer && mVertexBuffer )
						{
							...
							else if ( _stricmp(mSemantic,"position blendweights blendindices normal texcoord tangent binormal") == 0 )
							{
								NxuDeformVertex *vtx;
