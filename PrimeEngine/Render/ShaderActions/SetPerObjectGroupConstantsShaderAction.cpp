...
void SetPerObjectGroupConstantsShaderAction::bindToPipeline(Effect *pCurEffect /* = NULL*/)
{
	...
	errCode = sceGxmSetVertexUniformBuffer(pPSVitaRenderer->m_context, 1, s_pBuffer); // todo: use buferring
	PEASSERT(errCode == SCE_OK, "Error setting unfiorm buffer");
	errCode = sceGxmSetFragmentUniformBuffer(pPSVitaRenderer->m_context, 1, s_pBuffer); // todo: use buferring
	PEASSERT(errCode == SCE_OK, "Error setting unfiorm buffer");
