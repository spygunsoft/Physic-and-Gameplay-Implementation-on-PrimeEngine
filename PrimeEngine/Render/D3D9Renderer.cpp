...
void D3D9Renderer::setRenderTargetsAndViewportWithDepth(TextureGPU *pDestColorTex, TextureGPU *pDestDepthTex, bool clearRenderTarget, bool clearDepth)
{
	...
	if (pDestColorTex != 0)
	{
		...
		#ifdef _XBOX
			m_pD3D9Device->SetRenderTarget( 0, pDestColorTex->m_pEDRamColorRenderTargetSurface );
			m_pD3D9Device->SetDepthStencilSurface(pDestColorTex->m_pEDRamDSRenderTargetSurface ); // though m_pEDRamColorRenderTargetSurface references this depth surface, we still need to call this func and bind it
		#else
...
void D3D9Renderer::endRenderTarget(TextureGPU *pTex)
{
	...
	if (pTex)
		m_pD3D9Device->Resolve( D3DRESOLVE_RENDERTARGET0, NULL, pTex->m_pTexture,
...
void D3D9Renderer::setRenderTargetsAndViewportWithNoDepth(TextureGPU *pDestColorTex, bool clear)
{
	...
	if (pDestColorTex != 0)
	{
		...
		//todo: m_pD3D9Device->SetDepthStencilSurface(/*Do we need to set depth stencil here?*/NULL);
...
void D3D9Renderer::setDepthStencilOnlyRenderTargetAndViewport(TextureGPU *pDestDepthTex, bool clear)
{
	...
	if (pDestDepthTex != 0)
	{
		// texture is used
		#ifdef _XBOX
			m_pD3D9Device->SetRenderTarget( 0, pDestDepthTex->m_pEDRamColorRenderTargetSurface );
...
: IRenderer(context, width, height) // create window
{
	...
	//d3dpp.FrontBufferFormat       = D3DFMT_A8R8G8B8;
#else
...
void IRenderer::AcquireRenderContextOwnership(int &threadOwnershipMask)
{
	...
	D3D9Renderer *pD3D9Renderer = static_cast<D3D9Renderer*>(this);

	pD3D9Renderer->m_pD3D9Device->AcquireThreadOwnership();
#endif
...
void IRenderer::ReleaseRenderContextOwnership(int &threadOwnershipMask)
{
	...
	pD3D9Renderer->m_pD3D9Device->ReleaseThreadOwnership();
#endif
