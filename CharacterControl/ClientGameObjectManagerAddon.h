...
struct ClientGameObjectManagerAddon : public GameObjectManagerAddon
{
	...
	PE_DECLARE_IMPLEMENT_EVENT_HANDLER_WRAPPER(do_CreateImrodNPC);
	virtual void do_CreateImrodNPC(PE::Events::Event *pEvt);
	...
	void createImrodNPC(Vector3 pos, int &threadOwnershipMask);
	void createImrodNPC(Events::Event_CreateImrodNPC *pTrueEvent);
	...
	ImrodNPC *getTargetName(const char *name);
};
