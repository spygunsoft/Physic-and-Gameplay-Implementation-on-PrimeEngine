...
SoldierNPC::SoldierNPC(PE::GameContext &context, PE::MemoryArena arena, PE::Handle hMyself, Event_CreateSoldierNPC *pEvt) : Component(context, arena, hMyself)
{
	...
	for (int iSkin = 0; iSkin < numskins; ++iSkin)
	{
		...
		pPC->m_hAsset = pMeshInstance->m_hAsset;
		pPC->m_physicPlane = PE::Components::PhysicsComponent::physicPlane::sphere;
		pPC->m_sphereRadius = 1.0f;
		pPC->dynamicMesh = true;
		pMainSN->addComponent(hPC);

		m_pContext->getPhysicsManager()->physicsComponent.add(hPC);
	...
	StringOps::writeToString(pEvt->m_targetName, pSoldierMovementSM->m_curTargetName, 32);
	...
	//StringOps::writeToString(pEvt->m_targetName, pSoldierBehaviorSM->m_curTargetName, 32);
