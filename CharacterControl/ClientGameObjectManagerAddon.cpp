...
void ClientGameObjectManagerAddon::addDefaultComponents()
{
	...
	PE_REGISTER_EVENT_HANDLER(Event_CreateImrodNPC, ClientGameObjectManagerAddon::do_CreateImrodNPC);
...
void ClientGameObjectManagerAddon::do_CreateImrodNPC(PE::Events::Event *pEvt)
{
	assert(pEvt->isInstanceOf<Event_CreateImrodNPC>());

	Event_CreateImrodNPC *pTrueEvent = (Event_CreateImrodNPC*)(pEvt);

	createImrodNPC(pTrueEvent);
}
...
void ClientGameObjectManagerAddon::createImrodNPC(Vector3 pos, int &threadOwnershipMask)
{
	Event_CreateImrodNPC evt(threadOwnershipMask);
	evt.m_pos = pos;
	evt.m_u = Vector3(1.0f, 0, 0);
	evt.m_v = Vector3(0, 1.0f, 0);
	evt.m_n = Vector3(0, 0, 1.0f);
	...
	StringOps::writeToString("CharacterControl", evt.m_package, 255);
	StringOps::writeToString("mg34.x_mg34main_mesh.mesha", evt.m_gunMeshName, 64);
	StringOps::writeToString("CharacterControl", evt.m_gunMeshPackage, 64);
	StringOps::writeToString("", evt.m_patrolWayPoint, 32);
	createImrodNPC(&evt);
...
void ClientGameObjectManagerAddon::createImrodNPC(Event_CreateImrodNPC *pTrueEvent)
{
	PEINFO("CharacterControl: GameObjectManagerAddon: Creating CreateImrodNPC\n");

	PE::Handle hImrodNPC("ImrodNPC", sizeof(ImrodNPC));
	ImrodNPC *pImrodNPC = new(hImrodNPC) ImrodNPC(*m_pContext, m_arena, hImrodNPC, pTrueEvent);
	pImrodNPC->addDefaultComponents();

	// add the imrod as component to the ObjecManagerComponentAddon
	// all objects of this demo live in the ObjecManagerComponentAddon
	addComponent(hImrodNPC);
}
...
ImrodNPC *ClientGameObjectManagerAddon::getTargetName(const char *name) {
	...
	for (PrimitiveTypes::UInt32 i = 0; i < m_components.m_size; i++, pHC++) {
		...
		if (pC->isInstanceOf<ImrodNPC>())
		{
			ImrodNPC *pWP = (ImrodNPC *)(pC);
			if (StringOps::strcmp(pWP->m_targetName,name)==0) {
