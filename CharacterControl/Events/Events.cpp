...
PE_IMPLEMENT_CLASS1(Event_CreateImrodNPC, PE::Events::Event_CREATE_MESH);
...
void Event_CreateImrodNPC::SetLuaFunctions(PE::Components::LuaEnvironment *pLuaEnv, lua_State *luaVM)
{
	static const struct luaL_Reg l_Event_CreateImrodNPC[] = {
		{ "Construct", l_Construct },
		{ NULL, NULL } // sentinel
	};

	// register the functions in current lua table which is the table for Event_CreateImrodNPC
	luaL_register(luaVM, 0, l_Event_CreateImrodNPC);
}
...
int Event_CreateSoldierNPC::l_Construct(lua_State* luaVM)
{
	...
	if (!lua_isnil(luaVM, -numArgs)) {
		targetName = lua_tostring(luaVM, -numArgs--);
	}
	else {
		numArgs--;
	}
...
int Event_CreateImrodNPC::l_Construct(lua_State* luaVM)
{
	PE::Handle h("EVENT", sizeof(Event_CreateImrodNPC));
	...
	// this function should only be called frm game thread, so we can use game thread thread owenrship mask
	Event_CreateImrodNPC *pEvt = new(h) Event_CreateImrodNPC(pContext->m_gameThreadThreadOwnershipMask);
