...
int Event_CREATE_WAYPOINT::l_Construct(lua_State* luaVM)
{
	...
	//function for random nextWaypoint
	if(StringOps::length(nextWayPointName)>0)
	{
		
		char nextWaypoint[32];
		int choosenWaypoint;
		pEvt->m_count = 0;

		StringOps::writeToString(nextWayPointName, nextWaypoint, 32);

		char * parseNextWaypoint = strtok(nextWaypoint, ",");

		while (parseNextWaypoint != NULL) {

			StringOps::writeToString(parseNextWaypoint, pEvt->m_nextWaypointList[pEvt->m_count],32);
			pEvt->m_count++;
			parseNextWaypoint = strtok(NULL, ",");
		}

		choosenWaypoint = rand() % pEvt->m_count;

		StringOps::writeToString(pEvt->m_nextWaypointList[choosenWaypoint], pEvt->m_nextWaypointName, 32);
	}
	else {
...
: Component(context, arena, hMyself)
{
	...
	for (int i = 0; i < pEvt->m_count; i++) {
		StringOps::writeToString(pEvt->m_nextWaypointList[i], m_nextWaypointList[i], 32);
	}
	m_count = pEvt->m_count;
