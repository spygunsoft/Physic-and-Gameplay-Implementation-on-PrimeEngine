...
#ifdef _XBOX
char FAR * PASCAL FAR inet_ntoa (IN struct in_addr ina)
{
	unsigned char* bytes = (unsigned char *)&ina;
...
static int inet_global_gethostname(lua_State *L)
{
#if !defined(_XBOX) && !defined(SN_TARGET_PS3) && !PE_PLAT_IS_PSVITA
    char name[257];
    name[256] = '\0';
...
int pesocket_inet_aton(const char *cp, struct in_addr *inp)
{
	...
	* marvelously fast, but it works just fine.
	...
	#ifdef _XBOX
		// note %n does nto work on XBOX
		// will assign length to n
		// this means that this will only work if there are no empty characters in IP address
		r = sscanf(cp, "%u.%u.%u.%u", &a, &b, &c, &d);
		n = strlen(cp);
	#else
