...
/*pe-akovalovs begin*/
/* enables reuse of local address */
int opt_exclusiveaddr(lua_State *L, p_socket ps)
{
#if PE_PLAT_IS_IOS || PE_PLAT_IS_PS3 || PE_PLAT_IS_PSVITA || PE_PLAT_IS_ORBIS
	printf("SO_EXCLUSIVEADDRUSE is not available");
    lua_pushnumber(L, 1);
    return 1;
#else
...
{
#if !defined(_XBOX) && !PE_PLAT_IS_PS3 && !PE_PLAT_IS_PSVITA && !PE_PLAT_IS_ORBIS

    return opt_setboolean(L, ps, SOL_SOCKET, SO_DONTROUTE);
#else
	printf("SO_DONTROUTE option was not set\n");
	return 0;
#endif
}
...
static int opt_setmembership(lua_State *L, p_socket ps, int level, int name)
{
    ...
    if (!pesocket_inet_aton(lua_tostring(L, -1), &val.imr_multiaddr)) 
    ...
    if (strcmp(lua_tostring(L, -1), "*") &&
            !pesocket_inet_aton(lua_tostring(L, -1), &val.imr_interface)) 
...
static int opt_setboolean(lua_State *L, p_socket ps, int level, int name)
{
	...
	int existingVal = 0;
	int len = sizeof(existingVal);
	int res = getsockopt(*ps, level, name, (char *)&existingVal, &len); 

    res = opt_set(L, ps, level, name, (char *) &val, sizeof(val));
	return res;
}
